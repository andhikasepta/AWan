...
<script>
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      if (e.target && e.target.id === "newUserInput") {
        e.preventDefault();
        saveNewUser();
      }
    }
  ...
  function highlightText(text, keyword) {
    ...
    function editUser(el) {
      ...
      const regionEl = document.getElementById('editUserRegion');
      if (regionEl.tomselect) regionEl.tomselect.setValue(region);
      else regionEl.value = region;

      const areaEl = document.getElementById('editUserArea');
      const areaArray = area ? area.split(',').map(a => a.trim()) : [];
      if (areaEl.tomselect) {
          // If area is empty, setValue([]) clears it. Otherwise sets the array of values.
          areaEl.tomselect.setValue(areaArray);
      } else {
          Array.from(areaEl.options).forEach(opt => {
              opt.selected = areaArray.includes(opt.value);
          });
      }
    ...
    function saveUserAction() {
      ...
      const regionEl = document.getElementById('editUserRegion');
      const region = regionEl.tomselect ? regionEl.tomselect.getValue() : regionEl.value;
      
      const areaEl = document.getElementById('editUserArea');
      let area = "";
      if (areaEl.tomselect) {
          const vals = areaEl.tomselect.getValue();
          area = Array.isArray(vals) ? vals.join(', ') : vals;
      } else {
          area = Array.from(areaEl.selectedOptions).map(o => o.value).join(', ');
      }
  ...
  function saveNewUser() {
    ...
    const regionEl = document.getElementById("newUserRegion");
    const region = regionEl.tomselect ? regionEl.tomselect.getValue() : regionEl.value;
    
    const areaEl = document.getElementById("newUserArea");
    let area = "";
    if (areaEl.tomselect) {
        const vals = areaEl.tomselect.getValue();
        area = Array.isArray(vals) ? vals.join(', ') : vals;
    ...
    fetch("<?= base_url('dashboard/addUser') ?>", {
      ...
      .then(res => {
        if (res.success) {
          ...
          const regionEl = document.getElementById("newUserRegion");
          regionEl.value = "";
          // Clear Area TomSelect
          const areaElClear = document.getElementById("newUserArea");
          if (areaElClear.tomselect) {
              areaElClear.tomselect.clear();
          }
          loadUsers();
          refreshUserDropdown();
        } else {
      ...
      .then(data => {
        fetch("<?= base_url('dashboard/userList') ?>")
          ...
          .then(users => {
            ...
            const tsUser = new TomSelect("#edit_user", {
              create: false,
            ...
            const tsStatus = new TomSelect("#edit_status", {
              create: false,
              controlClass: 'ts-control no-arrow'
            });

            tsStatus.setValue(data.status_mutasi ?? "");
          });
        ...
        fetch("<?= base_url('perangkat/delete') ?>/" + id)
          ...
          .then(res => {
            if (res.success) {
              ...
              if (typeof showToast === 'function') {
                showToast("Data tidak ditemukan atau gagal menghapus", "error");
              } else {
  ...
  function openBulkEdit() {
    ...
    ids.forEach(id => {
      ...
      if (row) {
        ...
        listHtml += `<div class="py-1 border-b border-gray-200 last:border-0"><strong>${noreg}</strong> â€” ${nama}</div>`;
      }
    });
  ...
  // Global reference for regional dropdown refresh
  window.refreshRegionalTomSelects = function() {
      fetch("<?= base_url('dashboard/regionalList') ?>")
        ...
        .then(data => {
            ...
            let areas = [...new Set(data.map(item => item.area))].sort();

            document.querySelectorAll('.regional-dropdown').forEach(el => {
                ...
                let currentValue;
                if (isMultiple && el.tomselect) {
                    currentValue = el.tomselect.getValue();
                    if (typeof currentValue === 'string') currentValue = currentValue ? currentValue.split(',') : [];
                } else if (isMultiple) {
                    currentValue = Array.from(el.selectedOptions).map(o => o.value);
                } else {
                ...
                let placeholder = isRegion ? 'Pilih Region' : 'Pilih Area';
                if (!isMultiple) {
                ...
                // Restore value for non-TomSelect elements
                if (!isMultiple) {
                    if (currentValue) {
                        el.value = currentValue;
                    }
                } else {
                    // For multi-select, restore values before TomSelect init
                ...
                // Initialize TomSelect with checkbox plugin for multi-select area dropdowns
                if (isMultiple) {
                    new TomSelect(el, {
                        plugins: {
                            'checkbox_options': {
                        ...
                        onInitialize: function() {
                            // Restore values after TomSelect is initialized
      ...
      fetch('<?= base_url("dashboard/checkUpdates") ?>')
        ...
        .then(data => {
          ...
          if (data.latest !== lastKnownTimestamp || data.total !== lastKnownTotal) {
            // Don't auto-reload if user is actively editing in a modal
